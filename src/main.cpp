...
int main() {
  ...
  std::cout << "Ray from (10, 20) pointing east (0°): hit at distance " 
            << dist1 << std::endl;
  ...
  std::cout << "Ray from (50, 50) pointing east (0°): hit at distance " 
            << dist4 << " (world boundary)" << std::endl;

  // Ray with no collision
  Pose2D ray5(5.0f, 5.0f, deg2rad(225.0f)); // pointing southwest (away from world)
