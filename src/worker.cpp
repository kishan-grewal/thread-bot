...
WorkerBase::~WorkerBase() {
  ...
  // without this, if c++ kills a thread that has not exited, the program will literally just crash
  join();
}
...
void WorkerBase::start() {
    ...
    // else set running to true and create the thread, it does function thread_entry
    running_.store(true);
...
// the while loop has the atomic bool as a condition and will end in its own time, safely
...
void WorkerBase::join() {
    if (thread_.joinable()) {
        thread_.join();  // join is a please wait to the "this" thread, asking it to wait for our thread to exit
    }
}
...
// thread entry is literally a wrapper for whatever function the subclass wants to call in the thread
// we need this wrapper so that worker has absolute authority over when the thread is stopped
